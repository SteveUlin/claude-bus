...
- **`bin/`** — `bin/bus` is the unified C++ entry point for everything (broker, viewers, pane helpers, producers, consumers, lifecycle). `bin/agent-launch` is the only remaining shell helper, used by `bus spawn` to set an agent's identity (`claude --name NAME`) and resume its prior session (`--continue`) before `exec claude` — no session UUIDs; each agent's per-workspace cwd namespaces its sessions. Built by CMake from `src/` via `nix develop` + `cmake --build build`. Compiled binary is gitignored; the source-of-truth lives in `src/`.
- **`settings/`** — the canonical Claude Code project config: `settings/claude-settings.json` (the hook wiring) plus the hook scripts in `settings/hooks-shared/` (tracked, public). On every launch, `bin/agent-launch` **materializes this config from landed `main`** into each workspace's gitignored paths — `.claude/settings.json` (what Claude Code auto-loads) and `settings/hooks/` (where the `$CLAUDE_PROJECT_DIR/settings/hooks/…` refs resolve). It reads `main` via `jj --ignore-working-copy`, which is staleness-immune, so each agent's config equals landed `main` *by construction*. **Config therefore propagates via land-to-`main` + relaunch — not by editing a working copy live.** Edit the canonical (`settings/claude-settings.json`, `settings/hooks-shared/*`), commit, land to `main`, and relaunch to roll it out fleet-wide; never edit the materialized `.claude/settings.json` / `settings/hooks/` (they're regenerated each launch and gitignored). This replaced an earlier symlink-to-the-default-checkout, which silently went stale whenever the default workspace lagged `main`. (We also tried `CLAUDE_CONFIG_DIR=settings`; it forced a fresh login every session, so we use the project-level layering instead.)
...
- `zellij --layout layouts/fleet.kdl` — the working multi-agent layout: ops + primary + the named agent fleet (bast / kvothe / elodin). Each agent tab calls `bin/agent-launch NAME`, which resumes the agent's prior session with `claude --continue` (the most recent session in that agent's workspace cwd) so the fleet picks up where it left off after a host or zellij restart.
