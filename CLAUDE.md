...
- **`settings/`** — hook scripts shared by every agent in a session (e.g. `settings/hooks/log-event.sh`). The wiring file that references them is `.claude/settings.json` at the repo root, which Claude Code auto-loads as project-level config (layered on top of the user's `~/.claude/`). We tried `CLAUDE_CONFIG_DIR=settings` so the bus would have a fully isolated config; in practice it forced a fresh login every session, so we use the project-level layering instead.
