...
const Subcommand kSubcommands[] = {
    ...
    {"body", subBody, "fetch a record body by msg_id (no cursor advance)"},
    {"broadcast", subBroadcast, "fan out one body into many inboxes"},
    {"broker", subBroker, "broker daemon: foreground; flock-singleton"},
    {"enqueue", subEnqueue, "publish a record to a topic"},
    {"events", subEvents, "tail the bus event log"},
    {"fetch", subFetch, "pop the next record from a topic"},
    {"help", subHelp, "list subcommands"},
    {"inbox", subInbox, "tail-follow an agent's mailbox"},
    ...
    {"mail", subMail, "enqueue text to inbox-AGENT"},
    ...
    {"peek", subPeek, "read records from a topic without consuming"},
    {"send", subSend, "name→pane TTY write, holds per-pane flock"},
    {"slash", subSlash, "enqueue a slash command to commands-AGENT"},
    {"spawn", subSpawn, "open a new zellij tab for an agent"},
    {"state", subState, "broker's view of agent lifecycle"},
    {"topic", subTopic, "topic registry: create / list / show"},
